...
_VENDORED_LEAN_REL = "lean/LeanMining/NeuralGeometry/SubspaceOverlap.lean"
_LEAN_GITHUB_BASE = (
    "https://github.com/d3banjan/lazy-rudder-paper/blob/main/"
    + _VENDORED_LEAN_REL
)


def _scan_lean_decl_lines(lean_path: Path) -> dict[str, int]:
    """Return {decl_name: line_number} for theorem/lemma/def in a Lean file.

    Line numbers are 1-indexed (GitHub fragment convention #LXX).
    """
...
def build_lean_status():
    ...
    The vendored Lean source at lean/LeanMining/NeuralGeometry/SubspaceOverlap.lean
    is overwritten by `make sync-lean` from the monorepo canonical, so the
    public paper repo is self-contained.  manuscript/lean_status.tex is
    regenerated by `make lean-status` and is committed; it is the single
    authoritative source for the site dashboard.  Per-decl line numbers are
    scanned from the vendored file at site-data build time so each row in the
    site's Lean table can deep-link to the GitHub source.
    ...
    # decl_name → line_number lookup from vendored Lean source.
    # Use the same tex_name forward encoding as generate_lean_status.py so we
    # can reverse-map a \thmStatOfXxx macro back to its real decl name (the
    # CamelCase suffix loses underscore boundaries — e.g. `frobeniusSq_nonneg`
    # → `FrobeniusSqNonneg` is many-to-one if re-split naively).
    decl_lines = _scan_lean_decl_lines(Path(_VENDORED_LEAN_REL))

    def _tex_camel(decl_name: str) -> str:
        return "".join(w[:1].upper() + w[1:] for w in decl_name.split("_"))

    camel_to_decl = {_tex_camel(name): name for name in decl_lines}
    ...
    for m in re.finditer(
        ...
        # Resolve the CamelCase macro back to the real decl name via the
        # forward-encoded lookup; fall back to the naive snake conversion if
        # the decl scan didn't find it (vendored copy missing or stale).
        decl_name = camel_to_decl.get(camel_name) or re.sub(
            r"(?<!^)(?=[A-Z])", "_", camel_name
        ).lower()
        line_no = decl_lines.get(decl_name)
        entry = {"name": decl_name, "kind": "theorem", "status": status}
        if line_no is not None:
            entry["line"] = line_no
            entry["source_url"] = f"{_LEAN_GITHUB_BASE}#L{line_no}"
    ...
    return {
        ...
        "_source_url_base": _LEAN_GITHUB_BASE,
        "counts": {
            "proven": proven_count,
...
def main():
    ...
    Single source of truth is docs/assets/data/ (all client widgets fetch from
    here at runtime). docs/_data/ is reserved for files Jekyll templates need to
    consume server-side via site.data.* — currently only lean_status.json.
    """
