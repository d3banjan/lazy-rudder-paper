"""
Petri-dish DPO train — Pythia-160m, r=128.

Used to measure ΔW srank at d_in=768, to discriminate:
...
Mirrors dpo_leak_train_v2.py at 410m with minimal changes:
  - MODEL_DIR  → pythia-160m
  - OUT_DIR    → results/_leak_160m/v2
  - save_total_limit = 2 (disk hygiene; we only need checkpoint-800)

Wall estimate: ~15-20 min.
"""
...
MODEL_DIR  = BASE_DIR / "models" / "pythia-160m"
OUT_DIR    = BASE_DIR / "results" / "_leak_160m" / "v2"
...
def main():
    ...
    log.info(f"Pythia-160m: n_layers={n_layers}, d_model={d_model}")
    ...
    training_args = DPOConfig(
        ...
        max_length                  = 512,
        save_strategy               = "steps",
        save_steps                  = 400,       # only mid + final
        save_total_limit            = 2,         # disk hygiene
        logging_steps               = 10,
    ...
    log.info("Starting DPO training (Pythia-160m, r=128)...")
