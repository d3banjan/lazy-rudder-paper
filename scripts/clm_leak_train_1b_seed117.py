"""
γ₁ Step 2 — CLM LoRA r=128 on Pythia-1B (SEED=117 replicate, SGD control).
...
from _paths import MODELS_DIR, RESULTS_DIR, BASE_DIR  # noqa: E402
...
OUT_DIR   = RESULTS_DIR / "_leak_1b_seed117" / "v4"
...
# ── same partition fractions as CLM 410m ─────────────────────────────────────
P_GAUGE = 0.005
P_SRN   = 0.662
P_PS    = 0.333
...
def main():
    ...
    # ── base model fp32 for step-0 measurement ────────────────────────────────
    log.info("Loading Pythia-1B fp32 for step-0 L measurement...")
    base_for_step0 = AutoModelForCausalLM.from_pretrained(
        str(MODEL_DIR), torch_dtype=torch.float32, low_cpu_mem_usage=True
    ).to(DEVICE)
    log.info("Computing per-layer variance on hh-rlhf test prompts...")
    prompts = load_prompts(tokenizer, N_PROMPTS)
    variances = compute_per_layer_variance(base_for_step0, prompts)
    n_layers = len(base_for_step0.gpt_neox.layers)
    log.info(f"Pythia-1B: {n_layers} layers  hidden_size={base_for_step0.config.hidden_size}")
    log.info(f"Got variance for {len(variances)}/{n_layers} layers")
    ...
    log.info("Loading Pythia-1B fp16 for CLM training...")
    ...
    # ── L-measurement callback ────────────────────────────────────────────────
    class LeakCallbackCLM1B(tc.TrainerCallback):
        def __init__(self):
    ...
    log.info("Starting CLM training (Pythia-1B, r=128, 800 steps)...")
    ...
    L_range   = L_max - L_min if L_vals else float('nan')
    rel_range = L_range / L_mean if (L_vals and L_mean > 0) else float('nan')
    delta_iso = L_mean - L_ISO if L_vals else float('nan')
    ...
    (OUT_DIR / "summary_clm_1b.json").write_text(json.dumps(summary, indent=2))

    log.info(f"\n{'='*60}")
    log.info(f"CLM 1B SUMMARY")
    log.info(f"  L_mean  = {L_mean:.10f}")
    log.info(f"  Wall    = {(time.time()-t_start)/60:.1f} min")
    log.info(f"{'='*60}")

    print(f"\nCLM 1B done. Wall={summary['wall_sec']/60:.1f}min  loss={train_result.training_loss:.4f}")
